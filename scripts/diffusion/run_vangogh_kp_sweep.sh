#!/usr/bin/env bash
# Kp sweep on Van Gogh style erasure: CASteer (Kp only) vs our method
# (adaptive_kg), with the --diag per-step/per-block PID trace written for
# every arm. Style counterpart of scripts/diffusion/run_kp_sweep.sh -- same
# gains, same controller, same seed; only the prompts and the vector differ.
#
#   CASteer      kp in {2.0, 1.5, 1.0, 0.5}     ki = kg = kd = 0
#   Our method   kp in {2.0, 1.5, 1.0, 0.5}     ki = kg = 0.01, kd = 0.02
#
# Prompts: exp/datasets/eval/vangogh_kp_sweep.csv, 13 rows drawn from
# exp/datasets/eval/artists/big_artist_prompts.csv (the style eval set):
#   forget   0-4    the first 5 Van Gogh prompts        (case_number 20-24)
#   retain   5-12   the first 2 prompts of each other artist in the eval set
#                   -- Picasso, Rembrandt, Warhol, Caravaggio (case 0-1, 40-41,
#                   60-61, 80-81)
#
# Vector: results/sd14/steering_vectors_heldout/Van Gogh.pt, the contrastive
# vector fit against artists absent from the eval set, so the retain artists
# above are never seen by the vector (estimate_steering_vectors_style.py).
#
...
# --diag -- so its pid_records/ IS CASteer's e/u trace.
#
# Layout (gains in the directory name, as elsewhere under results/sd14):
#   results/sd14/vangogh_kp_sweep/
#       casteer_kp1.5/{0..12}/42-0.png + pid_records/000NN-42.csv
#       adaptive_kg_kp1.5_ki0.01_kg0.01_kd0.02/...
#       ...
#
# Usage:
#   bash scripts/diffusion/run_vangogh_kp_sweep.sh <gpu_ids>
#   bash scripts/diffusion/run_vangogh_kp_sweep.sh 1,5,0,4          # two arms per GPU
#   KPS="2.0" bash scripts/diffusion/run_vangogh_kp_sweep.sh 1,5    # only the Kp=2.0 arms
#   METHODS="casteer" bash scripts/diffusion/run_vangogh_kp_sweep.sh 1,5,0,4   # one controller only
#
...
GPU_IDS_STR=${1:?"Usage: bash $0 <gpu_ids>, e.g. 1,5,0,4"}
...
PROMPTS=exp/datasets/eval/vangogh_kp_sweep.csv
VECTORS="results/sd14/steering_vectors_heldout/Van Gogh.pt"
OUT=results/sd14/vangogh_kp_sweep
LOGS=logs/vangogh_kp_sweep
mkdir -p "$OUT" "$LOGS"
