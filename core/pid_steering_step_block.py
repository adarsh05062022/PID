...
the import of `VectorControl`. It is a drop-in alternative to
`core.controller.CrossAttentionOutputSteering`: same `VectorControl`
interface, same hooks (`core.diffusion_steering`), same steering-vector
files produced by `scripts/diffusion/estimate_steering_vectors.py`, same
cross-attention outputs of the same blocks. Only the scalar applied to the
steering direction changes -- CASteer's beta * max(<ca_X, ca_out>, 0)
becomes a PID law over that same projection. Select it with
`--controller pid` in `scripts/diffusion/run_with_steering.py`.

The docstring below is the original one. Modules it names that are not in
this repo (`core/pid_steering.py`, `core/pid_steering_step.py`,
`pid_records/`) live in the PID folder this was ported from; the CASteer
references in it are accurate for this repo.
...
class PIDSteering(VectorControl):
    """PID steering with per-block and cross-block integrators."""
    ...
    def forward(self, vector: torch.Tensor, diffusion_step: int, place_in_unet: str, block_index: int):
        ...
        # Contributes to I_global(t+1), NOT to the I_global read below.
        # Reduced to a scalar because blocks disagree in both sequence
        # length and width, so no per-token global term exists.
        self._step_errors.append(error.mean())

        p_out = self.kp_gain * error
        i_out = self.ki_gain * i_term
        d_out = self.kd_gain * d_term
        g_out = self.kg_gain * self._global_integral      # scalar, broadcasts
