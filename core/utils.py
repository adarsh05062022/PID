...
def init_pipeline_for_image_model(model: str) -> DiffusionPipeline:
    # No cache_dir override here on purpose: the previous './cache' pointed at
    # this repo's own dir on the NFS-mounted /scratch, which took ~40 minutes to
    # load a 4GB model. Leaving cache_dir unset falls back to HF's default
    # (~/.cache/huggingface/hub), which for this user is symlinked to local
    # /storage disk and already has every model in SUPPORTED_DIFFUSION_MODELS
    # cached -- loads in seconds instead.
    token = _get_hf_token()
    if model == 'sd14':
